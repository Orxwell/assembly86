...
.data # Section for declaring variables
	...
	msg2: .asciiz "El mas pequeño es: "     # The label 'msg2' represents an ASCII message
	msg3: .asciiz "Ningun numero es menor." # The label 'msg3' represents an ASCII message
...
.text # Section for the program code
	...
	ev1_layer1:
		ble $t0, $t1, ev1_moveA # $t0 <= $t1, CALL ev1_moveA
		bgt $t0, $t1, ev1_moveB # $t0 >  $t1, CALL ev1_moveB
	...
	ev2_layer1:
		ble $t1, $t2, ev2_moveB # $t1 <= $t2, CALL ev2_moveB
		bgt $t1, $t2, ev2_moveC # $t1 >  $t2, CALL ev2_moveC
	...
	ev3_layer2:
		ble $t3, $t4, result1 # $t3 <= $t4, CALL result1
		bgt $t3, $t4, result2 # $t3 >  $t4, CALL result2
